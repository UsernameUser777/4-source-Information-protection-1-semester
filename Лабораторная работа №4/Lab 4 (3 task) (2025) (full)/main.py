...
def mod_inverse(e, m):
    ...
    if gcd != 1:
        ...
        # print(f"Ошибка: e={e} и m={m} не взаимно просты, невозможно найти s_inv.")
        return None
...
def generate_prime_candidate(length):
    ...
    :param length: Битовая длина числа (например, 10 означает число от 512 до 1023).
...
def generate_prime_number(length=10):
    ...
    :param length: Битовая длина простого числа (по умолчанию 10 для простоты).
...
def generate_keys():
    ...
    p = generate_prime_number(10) # 10 бит - это 3..1023, подходит для демонстрации
    if not p:
...
def text_to_numbers(text, alphabet):
    ...
    Буква 'А' -> 0, 'Б' -> 1, ..., 'Я' -> 32.
    Это требуется по условию задачи для третьего способа шифрования.
...
def decrypt_action():
    ...
    # Получаем строку зашифрованных пар из поля ввода
...
root = tk.Tk()
root.title("Лабораторная работа 4 - ElGamal")
root.geometry("800x500") # Устанавливаем начальный размер окна

# --- Создание виджетов (элементов интерфейса) ---

# Метка и поле для ввода текста
input_label = ttk.Label(root, text="Введите текст (для шифрования) или пары (для дешифрования):")
input_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")

input_text = ttk.Entry(root, width=80)
...
encrypt_button = ttk.Button(root, text="Зашифровать (Текст -> Пары)", command=encrypt_action)
encrypt_button.grid(row=3, column=0, padx=10, pady=5, sticky="ew")

decrypt_button = ttk.Button(root, text="Дешифровать (Пары -> Текст)", command=decrypt_action)
