...
def generate_keys():
    ...
    # Для демонстрации используем битовую длину 16 (от 32768 до 65535).
    ...
    # Выберем e. Часто используется 65537.
    # e должно удовлетворять условиям: 1 < e < phi и gcd(e, phi) = 1 (взаимно просты)
    e = 65537
    # Проверим, подходит ли 65537. Если нет, выберем другое.
    if e >= phi or math.gcd(e, phi) != 1:
        # Если 65537 не подходит, выбираем другое
        e = 3
        while e < phi:
...
def encrypt_rsa(numbers, public_key):
    """
    Шифрует список чисел (кодов символов), с помощью открытого ключа RSA.

    :param numbers: Список целых чисел, представляющих символы (например, по позиции в алфавите).
...
def decrypt_rsa(encrypted_numbers, private_key):
    """
    Дешифрует список чисел, с помощью закрытого ключа RSA.

    :param encrypted_numbers: Список зашифрованных целых чисел.
...
def generate_keys_action():
    ...
    try:
        ...
        output_text.insert(tk.END, f"Закрытый ключ (d, n): {priv_key}\n")
...
root = tk.Tk()
root.title("Лабораторная работа 4 - RSA (Модифицированный)")
