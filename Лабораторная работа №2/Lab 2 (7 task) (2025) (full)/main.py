...
root.title("Шифр двойной перестановки")
...
key_frame.grid(row=2, column=0, padx=10, pady=5, sticky="w")
...
col_key_entry = ttk.Entry(key_frame, width=20)
...
row_key_entry = ttk.Entry(key_frame, width=20)
row_key_entry.grid(row=1, column=1, sticky="w")
...
# Размещаем кнопку (строка 3, столбец 0), прижимаем к востоку
encrypt_button.grid(row=3, column=0, padx=5, pady=10, sticky="e")
...
# Размещаем метку (строка 4, столбец 0)
...
# Размещаем поле вывода (строка 5, столбец 0, растягиваем на 2 столбца)
