...
def generate_random_key():
    """
    Генерирует случайные размеры таблицы (rows, cols).
    """
    # Минимальный размер 2x2, максимальный 6x6 (примерный разумный диапазон)
...
def generate_key_action():
    """
    Обработчик кнопки 'Сгенерировать размер'.
    Генерирует случайные размеры таблицы.
    """
    rows, cols = generate_random_key()
...
root.title("Шифр табличной маршрутной перестановки (Модифицированный)")
...
# Фрейм для ввода размеров таблицы и генерации
size_frame = ttk.Frame(root)
size_frame.grid(row=1, column=0, padx=10, pady=5, sticky="ew", columnspan=2)
...
# Кнопка генерации размеров
generate_key_button = ttk.Button(size_frame, text="Сгенерировать размер", command=generate_key_action)
generate_key_button.grid(row=0, column=4, padx=(10, 0), sticky="w")

# Фрейм для выбора маршрутов
route_frame = ttk.Frame(root)
route_frame.grid(row=2, column=0, padx=10, pady=5, sticky="ew", columnspan=2)
...
button_frame.grid(row=3, column=0, padx=10, pady=10, sticky="ew", columnspan=2)
...
# --- Настройка сетки ---
# Делаем так, чтобы строка 5 (где поле вывода) растягивалась при изменении размера окна
